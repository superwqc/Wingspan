...
namespace Restore.FIIS.BC.Configs
{
    public class RISConfigForm : RestoreForm
    {
        ...
        protected override object[] EncodeResponseArgs(FormResponse e)
        {
            switch (e.Title)
            {
                case RISConfigItemNames.OperateUserCode:
                case RISConfigItemNames.IsSimpleMode:
                case RISConfigItemNames.ExamLockValidTimeSpan:
                case RISConfigItemNames.FromPrintedToRejectedValidTimeSpan:
                case RISConfigItemNames.OnDutyTimeBegin:
                ...
                case RISConfigItemNames.AccessionNOGeneratorImplementList:
                case RISConfigItemNames.DataPrinterImplementList:
                    return this.Array2Json((IList<Restore.Utilities.CreateIniMgr.CreateIniItem>)e.Data);
                case RISConfigItemNames.AETitleListByWorkStation:
                    return this.Array2Json((IList<IV_DeviceInfo_LXUE>)e.Data);
                case RISConfigItemNames.UserList:
                    return this.Array2Json((IList<IUser_User_LXUE>)e.Data);
                default:
                    ErrorCodeHelper.Instance.ThrowException_Application(ECS3_Module.ConfigsModule, ECS4_ExceptionCode.InvalidRecord, string.Format("EncodeJSRequestArgs failed. Title = '{0}' , Data = '{1}'", e.Title, e.Data));
                    return null;
            }
        }
        ...
        private void NotifyRISConfigChanged()
        {
            if (OnRefreshControlData != null)
                OnRefreshControlData(this, new RefreshControlDataArgs(MessageLabels.RISConfigChanged));
        }
        ...
        {
            if (string.IsNullOrEmpty(this.AETitleByWorkStation))
                ErrorCodeHelper.Instance.ThrowException_Application(ECS3_Module.ConfigsModule, ECS4_ExceptionCode.NullObject, "请选择影像工作站的AEtitle");
            DcmServiceConfig.Instance.StorageSCUAETitle = this.AETitleByWorkStation;
        }
    }
}
